...
file_path_assets = filedialog.askopenfilename(title='导入资产负债表', filetypes=[('All Files', '*')])
...
# 利润表
file_path_profit = filedialog.askopenfilename(title='导入利润表', filetypes=[('All Files', '*')])
...
wb_report = load_workbook(filename="输出报表.xlsx")

sheets_report = wb_report.sheetnames

sheet_first_report = sheets_report[0]
sheet_second_report = sheets_report[1]
ws_report_assets = wb_report[sheet_first_report]
ws_report_profit = wb_report[sheet_second_report]
for k, v in assets_dict.items():
    ws_report_assets[k] = v
for k, v in profit_dict.items():
    ws_report_profit[k] = v

time.sleep(3)

# wb_report.save(r"D:\data\test\科目余额表报表\4\输出报表4.6.xlsx")
